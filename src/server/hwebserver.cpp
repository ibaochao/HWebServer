/**
 * @file hwebserver.cpp
 * @brief source files for HWebServer
*/
#include <unistd.h>
#include <fcntl.h>
#include <cstring>
...
HWebServer::HWebServer(const Config &cfg):
...
void HWebServer::init_event_mode(int trig_mode) {
 ...
 * @return 原来的文件状态标志
*/
int HWebServer::set_nonblocking(int fd) {
...
void HWebServer::add_client(int fd, const sockaddr_in &addr) {
    if (fd < 0) return;
    m_clients[fd] = std::make_shared<HttpConn>();
    m_clients[fd]->init(fd, addr);
    if (m_timeout > 0) {
        m_tm_heap->add(fd, m_timeout,
            std::bind(&HWebServer::close_conn, this, m_clients[fd]));
    }
    m_epoller->add_fd(fd, m_conn_event | EPOLLIN);
    set_nonblocking(fd);
}

void HWebServer::close_conn(std::shared_ptr<HttpConn> client) {
    if (!client) return;
    m_epoller->del_fd(client->get_fd());
    client->close_conn();
}

void HWebServer::extend_time(std::shared_ptr<HttpConn> client) {
    if (m_timeout > 0) {
        m_tm_heap->adjust(client->get_fd(), m_timeout);
    }
}

void HWebServer::send_error_msg(int fd, const char *msg) {
    if (fd < 0) return;
...
void HWebServer::deal_read(std::shared_ptr<HttpConn> client) {
    if (!client) return;
    extend_time(client);
    m_thread_pool->add_task(std::bind(&HWebServer::on_read, this, client));
}

void HWebServer::on_read(std::shared_ptr<HttpConn> client) {
    if (!client) return;
...
void HWebServer::on_process(std::shared_ptr<HttpConn> client) {
    if (client->process()) {
...
void HWebServer::deal_write(std::shared_ptr<HttpConn> client) {
    if (!client) return;
    extend_time(client);
    m_thread_pool->add_task(std::bind(&HWebServer::on_write, this, client));
}

void HWebServer::on_write(std::shared_ptr<HttpConn> client) {
    if(!client) return;
